...
//----------------
	...
	function mes(){
		...
		if (val == ""){
			...
			mTween(zf_kong,{top:50,left:100,opacity:100},800,'elasticOut');
			...
			if(val != "张兵军" && val != "顾峥燕" && val != "王松松" && val != "宋志银" && val != "耿太良" && val != "韩兆科" && val != "郑峰" && val != "华妍"){
				...
				mTween(zf_wu,{top:50,left:100,opacity:100},800,'elasticOut');
				...
				function zfCome() {
					for (var i = 0; i < imgs.length; i++) {
						if(val !== imgs[i].parentNode.getAttribute('name')){
							...
							mTween(imgs[i].parentNode,{left:450,top: 100,rotateY:0,scale:3},500,"linear");
							js.style.display = "block";
							console.log(js)
