...
function mTween(el, target, time, type, callBack) {
	...
	el.timer = setInterval(
		function() {
			...
			for (var s in b) {
				var val = (Tween[type](t, b[s], c[s], d)); //调用Tween对象对应的类型函数传入 对应的(t,b,c,d)拿到第t次应该到达的位置
				css(el, s, val);
			}
		},
		20
...
TouchEevent.prototype = {
	...
	fnEnd: function(e, index) {
		...
		if (!this.obj[index].isMove && this.tap) {
			this.tap.call(this.obj[index], index);
		}
	...
	document.addEventListener("touchmove",
		function(e) {
			if (e.touches.length >= 2 && isGesTure) {
				nowPinter = e.touches;
				scale = getDistance(nowPinter[0], nowPinter[1]) / getDistance(startPinter[0], startPinter[1]);
				rotate = getAngle(nowPinter[0], nowPinter[1]) - getAngle(startPinter[0], startPinter[1])
				e.scale = scale;
				e.rotation = rotate;
				obj.gesturemove && obj.gesturemove.call(el, e);
				//				el.innerHTML=e.touches.length
			}
		},
		false
	)
	document.addEventListener("touchend",
		function(e) {
			if (isGesTure) {
				...
				e.rotation = rotate;
				isGesTure = false;
				obj.gestureend && obj.gestureend.call(el, e);
			}
		},
		false
	);
